...
SSH (Secure Shell) 鍵は、パスワードなしでリモートサーバーに安全に接続できるようにします。
...
**パラメータの説明:**
- `-t ed25519`: 鍵タイプ (ED25519)
- `-f ~/.ssh/tmux_demo_key`: 出力ファイルパス
- `-C "tmux-demo"`: 識別用コメント

**生成時のプロンプト:**
1. ファイルの場所を確認 (Enterで承認)
2. パスフレーズを入力 (推奨) またはEnterでパスフレーズなし
3. パスフレーズを確認

**結果:**
- 秘密鍵: `~/.ssh/tmux_demo_key`
- 公開鍵: `~/.ssh/tmux_demo_key.pub`
...
**注意**: コマンドが見つからない場合は、設定 → アプリ → オプション機能 → OpenSSH クライアント で有効にしてください。

**鍵の場所:**

```text
C:\Users\YourName\.ssh\tmux_demo_key
C:\Users\YourName\.ssh\tmux_demo_key.pub
```
...
ssh-add C:\Users\YourName\.ssh\tmux_demo_key
```
...
一致しない場合は、[リモートホストへの鍵の追加](#リモートホストへの鍵の追加)を参照してください。
...
# または手動で
cat ~/.ssh/tmux_demo_key.pub | ssh user@host "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"
```
